...
export default class TrackOverviewCategorical {
  ...
  constructor(elm, attribute, filters) {
    ...
    const selectedFilters = ConditionBuilder.getSelectedNodes(
      attribute.id
    ).filters;
    elm.innerHTML = this.#filters
      .map((filter, index) => {
        filter.countLog10 = filter.count === 0 ? 0 : Math.log10(filter.count);
        filter.width = (filter.count / sum) * 100;
        filter.baseColor = util.colorTintByHue(
          category.color,
          (360 * index) / filters.length
        );
        const selectedClass =
          selectedFilters.indexOf(filter.node) !== -1 ? ' -selected' : '';
        return `
        <li class="track-filter-view _category-background-color${selectedClass}" style="width: ${width}%;" data-node="${
          filter.node
        }">
          <div class="labels">
            <p>
    ...
    elm
      .querySelectorAll(':scope > .track-filter-view')
      .forEach((elm, index) => {
        // reference
        ...
        elm.addEventListener('mouseenter', () => {
          const values = [];
          const userFilter = this.#userFilters?.find(
            userFilter => userFilter.node === filter.node
          );
            ...
            if (userFilter?.pvalue) {
              values.push({
                ...
                value:
                  userFilter.pvalue === 1
                    ? 1
                    : userFilter.pvalue.toExponential(3),
              });
            }
          } else {
            // has user filter
        ...
        elm.addEventListener('click', () => {
          if (elm.classList.contains('-selected')) {
            ...
            ConditionBuilder.addFilter(this.#attribute.id, filter.node);
          }
        });
      });
    ...
    DefaultEventEmitter.addEventListener(event.changeViewModes, e =>
      this.#update(e.detail)
    );
    DefaultEventEmitter.addEventListener(event.setUserFilters, e =>
      this.#plotUserIdFilters(e.detail)
    );
    DefaultEventEmitter.addEventListener(event.clearUserFilters, e =>
      this.#clearUserIdFilters(e.detail)
    );
  ...
  #update(viewModes) {
    ...
    const sum = this.#filters.reduce(
      (acc, filter) => acc + (isLog10 ? filter.countLog10 : filter.count),
      0
    );
    ...
    this.#filters.forEach(filter => {
      const width =
        ((isLog10
          ? filter.count === 0
            ? 0
            : Math.log10(filter.count)
          : filter.count) /
          sum) *
        100;
      filter.elm.style.backgroundColor = `rgb(${filter.baseColor
        .mix(
          App.colorSilver,
          1 - (isLog10 ? filter.countLog10 : filter.count) / max
        )
  ...
  #plotUserIdFilters(detail) {
    if (this.#attribute.id === detail.attributeId) {
      ...
      // mapping
      this.#filters.forEach(filter => {
        const userFilter = detail.filters.find(
          userFilter => userFilter.node === filter.node
        );
        if (userFilter?.mapped) {
